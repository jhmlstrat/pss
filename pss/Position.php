<?php
  ...
  class Position {
    ...
    public static function fromString($str) {
      ...
      $inst->when = When::fromString($whenStr);
      return $inst;
    }
    public function p($position) { $this->position = $position; }
    public function when($away,$home) { $this->when->away($away); $this->when->home($home); }
    public function toString() {
      return '{"position":{"pos":"' . $this->positionString($this->position) . '",' . substr($this->when->toString(),1,-1) . '}}';
    }
    ...
    public function stratString() {
      return '{"position":{"pos":"' . $this->positionString($this->position) . '","rating":"' . $this->rating . '","e":"' . $this->e . '","arm":"' . $this->arm . '","t":"' . $this->t . '","pb":"' . $this->pb . '"}}';
    }
    public function stratJson() {
      return json_decode($this->stratString());
    }
    public static function positionArray() {
      return array("DH","P","C","B1","B2","B3","SS","LF","CF","RF","PH","PR","OOG");
    }
    public static function position($positionString) {
      foreach (Position::positionArray() as $i => $p) {
        if ($p == $positionString) return $i;
      }
