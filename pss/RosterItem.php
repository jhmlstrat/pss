<?php
  ...
  class RosterItem 	{
    ...
    public static function processTradedFor($str) {
      ...
      for ($i=4; $i <= 13; $i++) {
        ...
        if ($inner[3] == MoveType::toString(MoveType::TradedFor)) {
          ...
          $move->moveType = MoveType::TradedFor;
          $inst->moves[] = $move;
          $rtn[] = $inst;
        }
      }
    ...
    public static function addMoveFileString(&$ri,$str) {
      ...
      for ($i=1; $i <= 3; $i++) {
        $inner = explode("~",$pieces[$i]);
        if ($ri->normalize($player->name) == $inner[0]) {
          $inj = new Injury;
          ...
          $ri->injuries[] = $inj;
          if ($inner[2] != 'none') {
            RosterItem::processMove($ri, $gameNo + 1,  MoveType::OnDL);
          }
        }
      }
      for ($i=4; $i <= 13; $i++) {
...
#print "processMove: " . $inner[0] . "  " . $inner[1] . "<br/>";
          RosterItem::processMove($ri, $gameNo, MoveType::fromString($inner[1]));
        }
        if ($ri->normalize($player->name) == $inner[2]) {
#print "processMove: " . $inner[2] . "  " . $inner[3] . "<br/>";
          RosterItem::processMove($ri, $gameNo, MoveType::fromString($inner[3]));
        }
      }
    }
    public static function processMove(&$ri, $gameNo, $moveType) {
      $doIt = true;
      if ($moveType == MoveType::OnDL or $moveType == MoveType::TradedFor) {
        for ($j=0; $j < count($ri->moves); $j++) {
          if ($ri->moves[$j]->gameNumber == $gameNo &&
               ($ri->moves[$j]->moveType == MoveType::OnDL ||
                $ri->moves[$j]->moveType == MoveType::TradedFor ))
              $doIt = false;
        }
      }
      if ($doIt) {
        ...
        if ($moveType == MoveType::TradedAway) $ri->endGame = $gameNo -1;
        $ri->moves[] = $move;
      }
    }
    public static function fromString($str) {
      ...
      for ($i=0; $i < count($injuries); $i++) {
        $inst->injuries[] = \Scoring\Injury::fromString(json_encode($injuries[$i]));
      }
    ...
    public function toString($includeStrat = false) {
      $rtn = '{"rosterItem":' . substr($this->player->toString($includeStrat),0,-1) . ',"team":"' . $this->team . '"';
      $rtn .= ',"moves":[';
    ...
    public function isMajors($gameNo) {
      ...
      if ($this->startGame <= $gameNo && $this->endGame >= $gameNo) {
        $lastMove = MoveType::ToMajors;
        for ($j=0; $j < count($this->moves); $j++) {
          if ($this->moves[$j]->gameNumber <= $gameNo) 
            $lastMove = $this->moves[$j]->moveType;
        }
        if ($lastMove == MoveType::ToMajors or $lastMove == MoveType::OffDL or
            $lastMove == MoveType::TradedFor) $rtn=true;
      }
      return $rtn;
    }
    public function isMinors($gameNo) {
      ...
      if ($this->startGame <= $gameNo && $this->endGame >= $gameNo) {
        ...
        if ($lastMove == MoveType::ToMinors or $lastMove == MoveType::OnDL) 
          $rtn=true;
      }
      return $rtn;
    }
      ...
      if ($this->startGame <= $gameNo && $this->endGame >= $gameNo) {
        ...
        if ($lastInjury->duration != 0) {
          ...
          for ($j=0; $j < count($restAfter); $j++) {
            if ($restAfter[$j] >= $lastInjury->gameNumber && $restAfter[$j] < $gameNo)
              $daysRest ++;
          }
          if (($lastInjury->gameNumber + $lastInjury->duration - $daysRest) >= $gameNo)
            $rtn = true;
        }
      }
      return $rtn;
    ...
    static function cmp($a, $b) {
      if ($a->team == $b->team) return strcmp($a->player->name, $b->player->name);
