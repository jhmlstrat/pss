...
class ResultsFile
{
    ...
    public function __construct($year = 2017)
    {
        ...
        $pss = glob('../data/'. $year . '/*.pss');
        if (count($pss) > 0) {
        } else {
            foreach ($this->_config['teams'] as $key => $team) {
                if (! file_exists(
                    '../data/' . $year .  '/' . $team['team'] . 'game'
                )
                ) {
                    return;
                }
            ...
            if (! file_exists('../data/'. $year . '/results.html')) {
                return;
            }
            ...
            for ($linei=0; $linei < count($lines); $linei++) {
                ...
                if (strpos(strtoupper($line), "<LI>") !== false) {
                    ...
                    for ($k=0; $k < count($gs); $k++) {
                        ...
                        //      $this->date_ = '';
                        //    }
                        //print (103-count($this->game[$t1])-$gn) . ' - ' . 
                        //    (103-count($this->game[$t2])-$gn) . "<br />";
                        $gm->team_[0] = $t1;
                        $gm->team_[1] = $t2;
                        //TBD
