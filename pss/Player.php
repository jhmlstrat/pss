...
class Player
{
    ...
    public static function fromString($str)
    {
        ...
        if (property_exists($play->player, 'positions')) {
            foreach ($play->player->positions as $pos) {
                array_push(
                    $inst->positions, Position::fromString(json_encode($pos))
                );
            }
        }
        //if (array_key_exists("strat",$play->player)) {
        if (property_exists($play->player, 'strat')) {
            ...
            for ($i = 0; $i < count($pos); $i ++) {
                array_push(
                    $inst->positionsPlayed, 
                    Position::fromStratString(json_encode($pos[$i]))
                );
            }
    ...
    public function toString($includeStrat = false)
    {
        ...
        if ($count > 0) {
            ...
            $oog = false;
            for ($i = 0; $i < $count; $i ++) {
                $posit=$this->positions[$i]->json()->position->pos;
                if ($posit == 'OOG') {
                    $oog = true;
                } else {
                    if (!$oog) {
